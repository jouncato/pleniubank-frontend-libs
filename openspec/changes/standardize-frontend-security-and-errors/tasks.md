...
- [ ] 1.3 Implementar endpoint de ticket WebSocket efímero, de un uso y con TTL corto para escenarios cross-site.
  - **No construido — y no bloquea el resto.** Lo que se implementó en su lugar (ya existente en `shared-http`, no nuevo de esta iteración) es auth por **cookie same-site directa**: `CoreWebSocketEventsService.connectCookieAuth()` conecta a `/ws/events` sin `?token=`, y `pleniubank-core` (`ws_events.py`) lee el JWT de la cookie `pleniu_access`/`pleniu_admin_access` cuando no hay query param. Verificado en vivo: conexión sin `?token=` en la URL, estado "conectado" en la UI. Esto cubre completamente el caso same-site (local, QA, y producción si algún día comparte dominio). El ticket efímero cross-site solo sería necesario si producción mantiene los subdominios separados actuales (`app.pleniu.co` / `backoffice.pleniu.co`) Y se quiere WS ahí sin exponer cookie cross-site — **decisión de arquitectura pendiente para cuando se aborde el cutover de producción**, no bloqueante hoy.
...
- [ ] 2.1 Hacer `httpOnlyCookie` la única estrategia posible en builds productivos de `shared-auth`.
  - **Parcial, decisión de scope pendiente.** Hoy coexisten ambas estrategias por diseño: `qa` (nueva configuración Angular, `environment.qa.ts`) usa `httpOnlyCookie`; `production` (`environment.ts`) sigue en `sessionStorage`, sin cambios. Esto fue deliberado para no arriesgar producción sin la infraestructura de subdominios lista (ver 1.1). **Pregunta abierta para Producto/Seguridad**: ¿el objetivo final es eliminar `sessionStorage` como opción incluso en builds de producción (bloquear el flag en el propio código, no solo por configuración), o basta con que producción nunca lo active mientras la infraestructura no esté lista? No se fuerza una respuesta aquí — ver `docs/session-strategy-rollout.md` sección "Checklist para activar producción".
...
> **PARCIAL (2026-07-19)** — verificación local completa; QA real y producción quedan
> pendientes de un despliegue real (fuera del alcance de una sesión local).
...
- [ ] 4.2 Ejecutar pruebas XSS/CSRF, inspección de storage/URLs/logs y regresión en ambos portales (más allá de la verificación manual local ya hecha).
- [x] 4.3 Publicar guía de migración y rollback para consumidores de las librerías.
  - `docs/session-strategy-rollout.md` (este repo): tabla de flags por entorno, pasos de prueba local, y checklist explícito para activar producción.
